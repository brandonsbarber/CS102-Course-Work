...
{
	if(argc < 4)	//Not all file names provides
	{
	...
	if(readFile(argv[1]))	//If there was an error in reading
	{
 	...
 	processTransactions(argv[2]);				//Parses commands
 	...
 	GMLWriter::write(argv[3],&users);					//Writes to file
 	return 0;
}
...
int readFile(char* fileName)
{
	...
	{
		cerr << "Cannot open input GML file. Please try again with valid file name." << endl;
  		return 1;
  	}
...
/**
 * Takes in a line from a file and uses that to parse out a User object
 * @param line the string from which to read
 * @return pointer to the User read from the string
 */
User* parseUser(string line)
{
...
int* findIndexes(string* names)
{
	...
	{
		throw invalid_argument("User cannot be found.");
	}
	return indexes;
}
...
void removeFriends(int* friends)
{
	...
	users.at(friends[1])->getFriends()->remove(friends[2]);
}
